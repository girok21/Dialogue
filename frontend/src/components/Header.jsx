import { Box, Button, Flex, IconButton, Image, Menu, MenuButton, MenuItem, MenuList, Portal, Text, useColorMode } from "@chakra-ui/react"
    ...
    >
        ...
        {!isAuthPath && <>
          ...
          <Menu >
            ...
            <MenuList maxWidth={'80px'}>
              <MenuItem onClick={toggleColorMode}>Switch appearance</MenuItem>
              <MenuItem onClick={()=>{navigate('/settings/profile')}}>Settings</MenuItem>
              <MenuItem onClick={()=>{dispatch(logout()); navigate('/auth/login')}}>Log out</MenuItem>
            </MenuList>
          </Menu>
        </>}
        {isAuthPath && 
          <IconButton
            ...
            icon={<SwitchThemeSvg colorMode={colorMode}/>}
...
const MoreSvg = ({outlineColor}) => (
  <svg id="more_icon" aria-label="More" fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24"><title>More</title><rect fill={outlineColor} height="2.5" rx="1.25" width="21" x="3" y="7"></rect><rect fill={outlineColor} height="2.5" rx="1.25" width="14" x="10" y="15"></rect></svg>
)

const SwitchThemeSvg = ({ colorMode }) => (colorMode === 'light' ?
